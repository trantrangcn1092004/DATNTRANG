...
orderController.payment = async (req, res) => {
	try {
		...
		if (!paying_method_id || !order_id) {
			return res.status(400).send("Thiếu mã đơn hàng hoặc phương thức thanh toán")
		}
		...
		console.log("login_user_id:", req.user.user_id)
		console.log("customer_id:", customer_id)
		console.log("order_id:", order_id)
		console.log("paying_method_id:", paying_method_id)
		console.log("purchase:", purchase)
		console.log("=========================")

		if (!purchase || purchase.length === 0) {
			return res.status(404).send("Không tìm thấy thông tin đơn hàng")
		}

		let purchaseData = purchase[0]

		if (paying_method_id == 1) {
			return res.render("./pages/order/momo", {
				header: header,
				user: header_user,
				formatFunction: formatFunction,
			...
			return res.render("./pages/order/atm", {
				header: header,
				user: header_user,
				formatFunction: formatFunction,
				purchase: purchaseData,
			...
			return res.render("./pages/order/credit", {
				header: header,
				user: header_user,
				formatFunction: formatFunction,
				purchase: purchaseData,
		...
		return res.status(500).send("Có lỗi xảy ra khi hiển thị trang thanh toán")
	}
}
